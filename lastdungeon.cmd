::      final dungeon - by bushty   ::
::                                                                       ::
::                                                                       ::
::     xpgain=((level-lvl)+lvl^2)*10+rnd %% (xpuntil/10)+1             ::
::                                                                       ::
::                                                                       ::
::                                                                       ::
::                                                                       ::
...
title Final Dungeon v1.0.0 - by Bushty

set run= entergame ::instanz  ::

:screen
cls
color 0f
mode con: cols=160 lines=63
echo.
...
echo.
set /p login= " "
if %login% EQU 1 set run=login
if %login% EQU 2 set run=createuser
if %login% GEQ 3 set run=entergame
goto screen
...
echo.
set /p cgender=
if %cgender% EQU 1 set gender=male   && set /a str+=1
if %cgender% EQU 2 set gender=female && set /a dex+=1
set run=choose_race
goto screen
...
echo.
set /p crace=
if %crace% EQU 1 set race=human  && set /a lck+=1
if %crace% EQU 2 set race=elf    && set /a dex+=1 && set /a str-=1
if %crace% EQU 3 set race=dwarf  && set /a str+=1 && set /a dex-=1
set run=choose_class
goto screen
...
echo.
set /p cclass=
if %cclass% EQU 1 set class=warrior && set /a str+=5 && set /a dex+=2
if %cclass% EQU 2 set class=hunter  && set /a dex+=5 && set /a str+=2 
if %cclass% EQU 3 set class=mage    && set /a int+=5 && set /a lck+=2
goto DATA_VALUES
...
if not exist "savedata\%name1%.bat" (
...
echo  5)  Max's Shop
echo.
echo  6)  Legend's Store
echo.
echo  7)  Smuggler
echo.
echo  8)  Trading Post
echo.
echo  9)  Infinity Bank
echo.
echo  10) Dungeon
echo.
echo  11) Skills
...
echo.
set /p x1=
if %x1% EQU 1 set run=woods
if %x1% EQU 2 set run=quests
if %x1% EQU 3 set run=weaponstore
if %x1% EQU 4 set run=armorstore
if %x1% EQU 5 set run=maxshopclosed
if %x1% EQU 6 set run=legendsstore
if %x1% EQU 7 set run=smuggler
if %x1% EQU 8 set run=post
if %x1% EQU 9 set run=bank
if %x1% EQU 10 set run=dungeon
if %x1% EQU 11 set run=skills
if %x1% EQU 97 set run=inventory
if %x1% EQU 98 set run=settings
if %x1% EQU 99 set run=entergame
goto screen
...
if %lvl% GTR 37 set /a lvl=37
if %lvl% GEQ 37 set xpuntil=None^^!
if %money% GTR 5000000 set /a money=5000000
if %playerxp% GTR 1000000 set /a playerxp=1000000
title Final Dungeon - Inventory
set destination2=inventory
...
echo.
set /p invencho1=
if %invencho1% EQU 5 goto SAVE_GAME_FILES
if %invencho1% EQU 1 set run=heal
if %invencho1% EQU 2 set run=weaponselect
if %invencho1% EQU 3 set run=Armorselect
if %invencho1% EQU 4 set run=viewlvls
goto screen
...
echo.
set /p invencho2=
if %invencho2% EQU back set run=inventory && goto screen



:quests
title Final Dungeon - Quest Hall
...
title Final Dungeon - Legend's Store
echo  --- Final Dungeon - Legend's Store ---
echo.
...
echo.º		         Owned: %la1%                º
echo.º                                               º
echo º 4) Alydril Armor:   	 Costs 120 Magic Orbsº
echo.º		   	 Owned: %la2%                º
...
title Final Dungeon - Weapon Shop

echo  -- Final Dungeon Weapon Shop --
echo.
echo  You currently have %money% Gold.
echo.
echo.
...
set levelneeded=15 )
if %w1% EQU 9 set run=weaponstorep2 && goto screen
if %w1% EQU exit goto SAVE_GAME_FILES
goto buynow
...
set levelneeded=20 )
if %w1% EQU 5 set run=weaponstore && goto screen
if %w1% EQU exit goto SAVE_GAME_FILES
goto buynow
...
set /p f2=
if %f2% EQU 1 set run=Woodcuttingmenu
if %f2% EQU 2 set run=Cookingmenu
if %f2% EQU 3 set run=Fishingmenu
if %f2% EQU 4 set run=Miningmenu
if %f2% EQU 5 set run=Smithingmenu
if %f2% EQU 6 set run=Thievingmenu
...
echo 2) Back
set /p f1=
if %f1% EQU 1 set run=Cookbow
if %f1% EQU 2 set run=skills
goto screen
...
echo 2) Back
set /p f1=
if %f1% EQU 1 set run=Fishingbow
if %f1% EQU 2 Set run=skills
goto screen
...
echo 2) Back
set /p f1=
if %f1% EQU 1 Set run=Smithingbow
if %f1% EQU 2 Set run=skills
goto screen
...
echo 2) Back
set /p f1=
if %f1% EQU 1 Set run=Thievingbow
if %f1% EQU 2 Set run=skills
goto screen
...
echo 2) Back
set /p f1=
if %f1% EQU 1 Set run=Woodcuttingbow
if %f1% EQU 2 Set run=skills
goto screen
...
:post
set destination1=post
title Final Dungeon - Trading Post
echo  --- Trading Post ---
echo.
echo  Gold: %money%
echo.
...
if %g2% EQU buy set run=buyitem
if %g2% EQU sell set run=sellitem
if %g2% EQU back set run=post
goto screen
...
if %g3% LSS 0 set run=post && goto screen
set /a cost=%g3%*!p%gcho%!
if %cost% GTR %money% goto nomoney
...
if %g4% EQU all set run=sellallitem
if %g4% LSS 0 set run=post 
if %g4% GTR !%gcho%! set run=sellallitem
...
if %a8% EQU d set run=depositgoldintobank
if %a8% EQU w Set run=withdrawgoldfrombank
goto screen
...
if %a6% LEQ 0 set run=bank && goto screen
if %a6% EQU all (
...
set run=bank
goto screen )
if %a6% GTR %money% set run=nomoney && set destination1=bank && goto screen 
...
if %a7% LEQ 0 set run=bank && goto screen
if %a7% EQU all (
...
set run=bank
goto screen )
if %a7% GTR !%bankgold%! set run=nomoney && set destination1=bank && goto screen
...
:maxshopopen
set /a ms1=0
title Final Dungeon - Max's Shop
echo  ---- Max's Shop ----
echo.
echo  What would you like to do?
echo.
...
echo.
set /p m1=
if %m1% EQU 1 set run=buymaxwep
if %m1% EQU 2 set run=buymaxarm
...
:buynow
if %lvl% LSS %levelneeded% goto NOTHIGHENOUGHLEVELTOBUY
...
title Final Dungeon - Armor Shop
echo  --- Final Dungeon - Armor Shop ---
echo.
echo  You currently have %money% Gold.
echo.
echo.
...
if %w1% EQU 9 set run=Armorstorep2 && goto screen
if %w1% EQU exit goto SAVE_GAME_FILES
goto buynow
...
if %w1% EQU 5 set run=Armorstore && goto screen
if %w1% EQU exit goto SAVE_GAME_FILES
goto buynow
...
echo.
set /p s1=
if %s1% EQU 1 set run=whoissmuggler
if %s1% EQU 2 set run=tradesmuggler
...
if %swordchoice2% EQU Armor set run=sellarmtosmug && goto screen
set run=selltosmug
goto screen
...
echo  2) No thanks.
set /p s3=
if %s3% EQU 1 set run=accepttradesmug && goto screen
if %s3% EQU 2 set run=declinesmug tomate && goto screen
...
echo.
set /p atkcho=
if %atkcho% EQU 1 set run=woods_atk
if %atkcho% EQU 2 set run=fightinginventory
if %atkcho% EQU 3 set run=flee
goto screen
...
echo.
timeout 2 >NUL
set /a miss=%random% %% (100+(%level%-%lvl%))+1 
set /a miss=%miss%+%dex%
if %miss% GEQ 50 (
...
echo.
timeout 1 >NUL
echo  It has %health% health left. )
if %miss% LSS 50 (
echo  You miss.
echo.
echo.
...
echo.
pause >NUL
if %health% LEQ 0 set run=woods_win_dropitem && goto screen
set run=woods_def
goto screen
...
echo.
timeout 2 >NUL
set /a miss=%random% %% (100+(%lvl%-%level%))+1 
set /a miss=%miss%-%dex%
if %miss% GEQ 50 (
...
timeout 1 >NUL
set /a hp=%hp% - %dmgnpc%
if %hp% LEQ 0 set run=die && goto screen
echo.
echo  You have %hp% health left. )
if %miss% LSS 50 (
echo  You dodge %npctype%'s attack.
echo.
echo.
echo. )
echo.
...
set /a xpgain=1
for /L %%i in (1,1,2) do set /a xpgain*=lvl
set /a xpgain=((%level%-%lvl%)+%xpgain%)*10+%random% %% (%xpuntil%/20)+1
set /a playerxp=%playerxp%+%xpgain%
set /a xpuntil=%xpuntil%-%xpgain%
echo  You beat %npctype%.
echo.
timeout 1 >NUL
echo  You recieve %word% %drop%.
echo  You gain %goldgain% gold.
echo  You gain %xpgain% Experience.
echo.
echo.
pause >NUL
...
if %xpuntil% LEQ 0 set destination=woods && goto levelup
goto woods
...
echo  4) Change Armor
echo.
echo  5) Return to battle
echo.
set /p invencho=
if %invencho% EQU 1 set run=drinkpotion
if %invencho% EQU 2 set run=eatfood
if %invencho% EQU 3 set run=weaponselect
if %invencho% EQU 4 set run=armorselect
if %invencho% EQU 5 set run=woods_atk
goto screen
...
:weaponselect
title Final Dungeon - Weapon Select
...
echo  You currently have a %swordtype% %skind% equipped.
echo.
echo  Select a sword to equip, or go back.
echo.
...
if %wepsel% EQU 15 set run=%destination2% && goto screen
...
:Armorselect
title Final Dungeon - Armor Select
...
if %armsel% EQU 15 set run=%destination2% && goto screen
...
echo  ---------  WARNING  ---------
echo.
echo  THE DUNGEON IS VERY DANGEROUS. YOU WILL LOSE ALL YOUR ITEMS IF YOU ARE NOT PROPERLY PREPARED
echo  YOU WILL NOT BE ABLE TO FLEE OR ACESS YOUR INVENTORY DURING BATTLE. SO BE PREPARED!
echo.
...
echo.
set /p str1=
if %str1% EQU 1 goto instanz
if %str1% EQU 2 goto SAVE_GAME_FILES

:instanz
color 0f
set /a npc= %random% %% 3+1
if %npc% EQU 1 goto goblin
if %npc% EQU 2 goto orc
if %npc% EQU 3 goto skeleton

:goblin
set /a npc=%random% %% 3+1
if %npc% EQU 1 set npctype=Goblin witchdoctor && goto magic
if %npc% EQU 2 set npctype=Goblin hunter && goto ranged
if %npc% EQU 3 Set npctype=Goblin rogue && goto melee

:orc
set /a npc=%random% %% 3+1
if %npc% EQU 1 set npctype=Orc shaman && goto magic
if %npc% EQU 2 set npctype=Orc archer && goto ranged
if %npc% EQU 3 Set npctype=Orc warrior && goto melee

:skeleton
set /a npc=%random% %% 3+1
if %npc% EQU 1 set npctype=Skeleton mage && goto magic
if %npc% EQU 2 set npctype=Skeleton ranger && goto ranged
if %npc% EQU 3 Set npctype=Skeleton rogue && goto melee
...
:dungeon_fight
if %killcount% EQU 10 set run=dungeon_boss && goto screen
title Final Dungeon - Dungeon
echo.
echo  You continue into the dungeon...
echo.
echo.
timeout 2 >NUL
echo  You encounter a lvl %level% %npctype%.
echo.
echo.
timeout 2 >NUL
set run=dungeon_atk
...
echo.
pause >NUL
if %health% LEQ 0 set run=CHECKFORLVL_STR
set run=def_dungeon
goto screen

:def_dungeon
if %npcclass% EQU magic set /a dmgnpc=%random%*200/32767+1
if %npcclass% EQU ganged set /a dmgnpc=%random%*200/32767+1
if %npcclass% EQU melee set /a dmgnpc=%random%*200/32767+1
if %armtype% EQU Cloth set /a dr=%random%*10/32767+1
if %armtype% EQU Chain set /a dr=%random%*20/32767+1
if %armtype% EQU Bronze set /a dr=%random%*40/32767+1
if %armtype% EQU Iron set /a dr=%random%*65/32767+1
...
if %killcount% EQU 8 set /a hp=%orighp%
if %killcount% EQU 10 set /a hp=%orighp%
if %xpuntil% LEQ 0 set destination=instanz && goto levelup
goto instanz
...
echo.
timeout 2 >nul
set run=dungeon_boss_fight_atk
goto screen


:dungeon_boss_fight_atk
if %swordtype% EQU Your set /a damage=%random%*50/32767+1
if %swordtype% EQU Wooden set /a damage=%random%*100/32767+1
if %swordtype% EQU Stone set /a damage=%random%*125/32767+1
if %swordtype% EQU Bronze set /a damage=%random%*150/32767+1
...
echo.    
pause >nul
if %health% LEQ 0 goto dungeon_boss_fight_win
set run=dungeon_boss_fight_def
goto screen

:dungeon_boss_fight_def
...
if %hp% LEQ 0 set run=die && goto screen
echo.
echo  You have %hp% health left.set /a miss=%random% %% (100+(%lvl%-%level%))+1 
set /a miss=%miss%-%dex%
if %miss% GEQ 50 (
echo  The %npctype% deals %dmgnpc% damage to you.
echo.
...
set /a xpgain=1
for /L %%i in (1,1,2) do set /a xpgain*=lvl
set /a xpgain=((%level%-%lvl%)+%xpgain%)*10+%random% %% (%xpuntil%/10)+1
set /a playerxp=%playerxp%+%xpgain%
set /a xpuntil=%xpuntil%-%xpgain%
echo.
echo  You won the battle against %npctype%.
echo.
echo  You recieve an Omega Maul.
echo  You recieve Runic Armor.
echo  You gain %goldgain% Gold.
echo  You gain %xpgain% Experience.
echo.
pause>nul
...
:levelup
set /a lvl=%lvl%+1
set /a xpuntil=%lvl%*%lvl%*15/10+%playerxp%*15/10
...
echo.
echo.
echo You have leveled up. You are now level %lvl%.
echo You now have %hp% health.
echo.
pause>nul
set run=%destination%
...
:: ((%level%-%lvl%)+%xpgain%)*10+%random% %% (%xpuntil%/20)+1 ::
...
set ttllvl=13
set run=SAVE_GAME_FILES
goto screen



:: speichert spielstand in externer batch datei ::
:SAVE_GAME_FILES
...
echo.
set /p scho=
if %scho% EQU 1 set run=color
if %scho% EQU 2 set run=namechange
if %scho% EQU 3 set run=passchange
if %scho% EQU 4 set run=verifyadmin
if %scho% EQU 5 set run=SAVE_GAME_FILES
goto screen
...
echo.
set /p colorcho=
if %colorcho% EQU exit set run=settings goto screen
color %colorcho%
set run=color
goto screen
